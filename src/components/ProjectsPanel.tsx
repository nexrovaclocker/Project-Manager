...
export function ProjectsPanel() {
    ...
    return (
        <div className="flex flex-col h-full w-full bg-transparent text-white relative rounded-2xl overflow-hidden glass-panel z-10">
            <div className="absolute top-0 left-0 w-full h-1 bg-[#f97316] shadow-[0_0_10px_#f97316]"></div>
            ...
            <div className="flex items-center justify-between p-6 border-b border-[#f97316]/20 bg-[#1E1E2E] relative">
                <div className="flex items-center gap-4 relative z-10">
                    ...
                    <div className="hidden md:flex ml-8 bg-black/20 rounded-lg p-1 border border-[#f97316]/20">
                        <button
                            onClick={() => setView('MY_PROJECTS')}
                            className={`px-4 py-1.5 text-[10px] font-bold tracking-widest uppercase transition-all rounded-md ${view === 'MY_PROJECTS' ? 'bg-[#f97316] text-white shadow-sm' : 'text-[#94A3B8] hover:text-white hover:bg-white/5'}`}
                        >
                            Active_Nodes
                        </button>
                        <button
                            onClick={() => setView('PROJECT_TRACKER')}
                            className={`px-4 py-1.5 text-[10px] font-bold tracking-widest uppercase transition-all rounded-md ml-1 ${view === 'PROJECT_TRACKER' ? 'bg-[#f97316] text-white shadow-sm' : 'text-[#94A3B8] hover:text-white hover:bg-white/5'}`}
                        >
                            Global_Stream
                        </button>
                    </div>
            ...
            <div className="flex flex-col lg:flex-row flex-1 overflow-hidden">
                {view === 'MY_PROJECTS' ? (
                    <>
                        <div className="w-full lg:w-1/3 border-r border-[#f97316]/20 flex flex-col overflow-y-auto scrollbar-custom bg-[#1E1E2E]/40">
                            {isAdmin && (
                                <div className="p-6 border-b border-[#f97316]/20 bg-[#f97316]/5 relative overflow-hidden group">
                                    <h3 className="text-[10px] font-bold tracking-widest text-[#f97316] mb-4 uppercase flex items-center gap-2">
                                    ...
                                    <form onSubmit={handleCreateProject} className="flex flex-col gap-3 relative z-10">
                                        ...
                                        {pmMsg && <div className="text-[10px] text-[#f97316] text-center uppercase tracking-widest font-bold mt-2">{pmMsg}</div>}
                                    </form>
                                </div>
                            )}
                            ...
                            <div className="flex-1 flex flex-col p-3 gap-3">
                                {projects.length === 0 ? (
                                    <div className="p-6 text-[10px] text-[#94A3B8] tracking-widest text-center mt-4 border border-dashed border-[#f97316]/20 rounded-2xl">
                                        NULL_PROJECT_ARRAY
                                    </div>
                                ) : (
                                    projects.map(project => (
                                        <button
                                            ...
                                            className={`text-left p-5 rounded-2xl border transition-all duration-300 relative group/item ${activeProjectId === project.id ? 'bg-[#f97316]/10 border-[#f97316] shadow-[0_0_15px_rgba(249, 115, 22,0.2)]' : 'bg-transparent border-[#f97316]/10 hover:border-[#f97316]/40'}`}
                                        >
                                            <div className="absolute left-0 top-1/2 -translate-y-1/2 w-1 h-2/3 bg-[#f97316] opacity-0 group-hover:opacity-100 transition-opacity rounded-r"></div>
                                            ...
                                            <div className={`font-bold tracking-widest text-xs uppercase flex items-center justify-between ${activeProjectId === project.id ? 'text-white' : 'text-[#94A3B8]'}`}>
                                                ...
                                                {activeProjectId === project.id && <span className="w-1.5 h-1.5 rounded-full bg-[#f97316] shadow-[0_0_8px_#f97316] animate-pulse"></span>}
                                            </div>
                        ...
                        <div className="w-full lg:w-2/3 flex flex-col bg-transparent overflow-y-auto scrollbar-custom relative">
                            {activeProject ? (
                                <div className="p-8 flex flex-col h-full z-10">
                                    <div className="mb-8 pb-8 border-b border-[#f97316]/10">
                                        ...
                                        <div className="mt-8 pt-8 border-t border-[#f97316]/10 grid grid-cols-1 md:grid-cols-2 gap-8">
                                            <div>
                                                ...
                                                <div className="flex flex-wrap gap-2">
                                                    {activeProject.members.length === 0 ? (
                                                        ...
                                                        activeProject.members.map(m => (
                                                            <span key={m.id} className="group text-[10px] bg-black/40 border border-[#f97316]/20 px-3 py-1.5 rounded-full tracking-widest uppercase flex items-center gap-3">
                                    ...
                                    <div className="flex-1 flex flex-col min-h-0 relative z-10">
                                        ...
                                        <div className="space-y-4 mb-8 flex-1 overflow-y-auto scrollbar-custom pr-2">
                                            {activeProject.todos.length === 0 ? (
                                                <div className="text-[10px] text-[#94A3B8] tracking-widest text-center py-10 border border-dashed border-[#f97316]/10 rounded-2xl bg-black/20">
                                                    NULL_DIRECTIVES
                                                </div>
                                            ) : (
                                                activeProject.todos.map(todo => (
                                                    <div key={todo.id} className="flex items-start justify-between group/todo p-5 rounded-2xl border border-[#f97316]/10 bg-black/40 hover:border-[#f97316]/30 transition-all">
                                                        <label className="flex items-start gap-4 cursor-pointer flex-1">
                                                            <div className="relative flex items-center pt-0.5 shrink-0">
                                                                ...
                                                                />
                                                                <div className="w-5 h-5 rounded-lg border border-[#f97316]/30 flex items-center justify-center peer-checked:bg-[#f97316] peer-checked:border-[#f97316] transition-all">
                                                                    {todo.checked && (
                                        ...
                                        <form onSubmit={addTodo} className="mt-auto pt-6 border-t border-[#f97316]/10 flex gap-4">
                                            <input
                                                type="text"
                    ...
                    <div className="w-full flex flex-col p-8 overflow-y-auto scrollbar-custom bg-[#0A0A0B]">
                        <h3 className="text-xl font-bold tracking-widest text-white mb-10 uppercase border-l-4 border-[#f97316] pl-6">
                        ...
                        {projects.length === 0 ? (
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                {projects.map(project => {
                                    ...
                                    return (
                                        <div key={project.id} className="glass-panel p-8 group hover:border-[#f97316] transition-all">
                                            ...
                                            <div className="w-full h-1 bg-white/5 mb-8 rounded-full overflow-hidden">
                                                <div
                                                    className="h-full bg-[#f97316] shadow-[0_0_10px_#f97316] transition-all duration-1000"
                                                    style={{ width: `${percent}%` }}
                                                />
                                            </div>
                                            ...
                                            <div className="space-y-3">
                                                {project.todos.slice(0, 3).map(todo => (
                                                    <div key={todo.id} className="flex items-center gap-3 text-[11px] text-[#94A3B8] tracking-wide">
                                                        <div className={`w-1.5 h-1.5 rounded-full ${todo.checked ? 'bg-[#f97316]' : 'border border-[#f97316]'}`}></div>
                                                        <span className={todo.checked ? 'line-through opacity-40' : ''}>{todo.text}</span>
                                                    </div>
                                                ))}
                                                {total > 3 && <div className="text-[9px] text-[#f97316] font-bold tracking-widest ml-4 mt-2">+{total - 3} MORE_TASKS</div>}
                                            </div>
                                        </div>
                                    )
                                })}
