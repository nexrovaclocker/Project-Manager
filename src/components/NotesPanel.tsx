        ...
        <div className="flex flex-col h-full w-full bg-[var(--color-bg-dark)] text-[var(--color-text-primary)] relative">
            <div className="flex items-center justify-between p-4 border-b border-[var(--color-panel-border)]">
                ...
                <button
                    ...
                    className="text-xs font-bold tracking-widest px-2 py-1 border border-[var(--color-panel-border)] hover:border-[var(--color-green-accent)] hover:text-[var(--color-green-accent)] transition-colors text-[var(--color-text-secondary)]"
                >
                    + NEW_RECORD
                </button>
            </div>
            ...
            {isCreatingNote && (
                <form onSubmit={createNote} className="p-4 border-b border-[var(--color-green-accent)] flex gap-2 bg-[var(--color-panel)]">
                    <input
                        autoFocus
                        className="flex-1 bg-transparent border-b border-[var(--color-panel-border)] text-sm focus:outline-none focus:border-[var(--color-green-accent)] px-2"
                        placeholder="RECORD_TITLE..."
                        value={newNoteTitle}
                        onChange={(e) => setNewNoteTitle(e.target.value)}
                    />
                    <button type="submit" className="text-xs text-[var(--color-green-accent)] tracking-widest px-2">CREATE</button>
                ...
                {notes.map((note) => (
                    <button
                        ...
                        className={`flex-shrink-0 px-4 py-3 text-xs font-bold tracking-widest border-r border-[var(--color-panel-border)] transition-colors ${activeNoteId === note.id
                            ? 'bg-[var(--color-panel)] text-[var(--color-green-accent)] border-b-2 border-b-[var(--color-green-accent)]'
                    ...
                    <div className="space-y-6 max-w-3xl mx-auto">
                                    ...
                                    {block.type === 'text' ? (
                                        editingBlockId === block.id ? (
                                            <div className="space-y-2">
                                                <textarea
                                                    className="w-full text-sm bg-[var(--color-bg-dark)] border border-[var(--color-green-accent)] p-2 focus:outline-none resize-none"
                                                ...
                                                <div className="flex gap-2">
                                                    <button onClick={saveEdit} className="text-xs text-[var(--color-green-accent)] tracking-widest border border-[var(--color-green-accent)] px-2 py-1">SAVE</button>
                                            ...
                                            >
                                                ...
                                                <button
                                                    ...
                                                    className="absolute top-0 right-0 opacity-0 group-hover:opacity-100 text-[10px] text-[var(--color-green-accent)] tracking-widest border border-[var(--color-green-accent)] px-1 transition-opacity"
                                                >
                                                    EDIT
                                                </button>
                                            </div>
                        ...
                        <div className="mt-8 pt-8 border-t border-[var(--color-panel-border)] grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div className="space-y-2">
                                <textarea
                                    rows={2}
                                    className="w-full text-sm bg-[var(--color-bg-dark)] border border-[var(--color-panel-border)] p-2 focus:border-[var(--color-green-accent)] focus:outline-none resize-none"
                                ...
                                <button
                                    ...
                                    className="w-full text-xs font-bold tracking-widest py-1 border border-[var(--color-panel-border)] hover:text-[var(--color-green-accent)] hover:border-[var(--color-green-accent)] transition-colors"
                                >
                                    Add Note
                                </button>
                            </div>
                            ...
                            <div className="space-y-2">
                                <input
                                    type="text"
                                    className="w-full text-sm bg-[var(--color-bg-dark)] border border-[var(--color-panel-border)] p-2 focus:border-[var(--color-green-accent)] focus:outline-none"
